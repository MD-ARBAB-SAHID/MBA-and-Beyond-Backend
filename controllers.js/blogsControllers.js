...
const postBlog = async (req, res, next) => {
  ...
  if (!blog) return next(new HttpError("Blog not found to update", 404));

  //checking if authorise owner has requested to post the blog
  if (blog && blog.owner.toString() !== existingUser.id.toString())
    return next(
      new HttpError("Cannot post blog as you are not the owner.", 401)
    );
...
const allBlogs = async (req, res, next) => {
  ...
  try {
    blogs = await Blog.find(
      { blogStatus: "posted" },
      { title: true, thumbnail: true, owner: true, blogStatus: true }
    ).populate("owner", "name");
  } catch (err) {
...
const blogDetails = async (req, res, next) => {
  ...
  try {
    blogs = await Blog.find({ _id: blogId, blogStatus: "posted" })
      .populate("owner", "name")
      .populate({
        path: "comments",
        populate: {
...
const secureBlogDetails = async (req, res, next) => {
  ...
  //fetching details of particular blog from database
  try {
    blogs = await Blog.find({ _id: blogId, blogStatus: incomingBlogStatus })
      .populate("owner", "name")
      .populate({
        path: "comments",
        populate: {
...
const deleteBlog = async (req, res, next) => {
  ...
  //if blog is not found
  if (!blog) return next(new HttpError("Blog not found to delete", 404));

  //checking if authorise owner has requested to delete the blog
  if (blog && blog.owner.toString() !== existingUser.id.toString())
    return next(
      new HttpError("Cannot delete blog as you are not the owner.", 401)
    );
...
const myBlogs = async (req, res, next) => {
  ...
  try {
    existingUser = await User.findById(tokenUserId, { blogs: true }).populate({
      path: "blogs",
      match: {
        blogStatus: blogStatus,
      },
...
const updateBlog = async (req, res, next) => {
  ...
  if (blog && blog.owner.toString() !== existingUser.id.toString())
    return next(
      new HttpError("Cannot update blog as you are not the owner.", 401)
    );
